...
void ServerConfigDialog::initControls()
{
  ...
  //block for wifiscreenserver
  m_enableFileTransfers.setEnabled(false);
  m_acceptHttpConnections.setEnabled(false);
  m_acceptRfbConnections.setEnabled(false);
  m_blockLocalInput.setEnabled(false);
  m_showTrayIcon.setEnabled(false);
  m_pollingInterval.setEnabled(false);
  m_grabTransparentWindows.setEnabled(false);
  m_useMirrorDriver.setEnabled(false);
  m_readOnlyPassword.setEnabled(false);
}
